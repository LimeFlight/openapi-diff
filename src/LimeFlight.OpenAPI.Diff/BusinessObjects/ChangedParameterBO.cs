...
namespace LimeFlight.OpenAPI.Diff.BusinessObjects
{
    public class ChangedParameterBO : ComposedChangedBO
    {
        ...
        protected override List<ChangedInfoBO> GetCoreChanges()
        {
            ...
            if (IsChangeRequired)
                returnList.Add(new ChangedInfoBO(elementType, changeType, "Required", OldParameter?.Required.ToString(),
                    NewParameter?.Required.ToString()));

            if (IsDeprecated)
                returnList.Add(new ChangedInfoBO(elementType, changeType, "Deprecation",
                    OldParameter?.Deprecated.ToString(), NewParameter?.Deprecated.ToString()));

            if (ChangeStyle)
                returnList.Add(new ChangedInfoBO(elementType, changeType, "Style", OldParameter?.Style.ToString(),
                    NewParameter?.Style.ToString()));

            if (ChangeExplode)
                returnList.Add(new ChangedInfoBO(elementType, changeType, "Explode", OldParameter?.Explode.ToString(),
                    NewParameter?.Explode.ToString()));

            if (ChangeAllowEmptyValue)
                returnList.Add(new ChangedInfoBO(elementType, changeType, "AllowEmptyValue",
                    OldParameter?.AllowEmptyValue.ToString(), NewParameter?.AllowEmptyValue.ToString()));
