...
namespace LimeFlight.OpenAPI.Diff.BusinessObjects
{
    public class ChangedHeaderBO : ComposedChangedBO
    {
        ...
        {
            if (!Required && !Deprecated && !Style && !Explode) return new DiffResultBO(DiffResultEnum.NoChanges);
            if (!Required && !Style && !Explode) return new DiffResultBO(DiffResultEnum.Compatible);
        ...
        protected override List<ChangedInfoBO> GetCoreChanges()
        {
            ...
            if (Required)
                returnList.Add(new ChangedInfoBO(elementType, changeType, "Required", OldHeader?.Required.ToString(),
                    NewHeader?.Required.ToString()));

            if (Deprecated)
                returnList.Add(new ChangedInfoBO(elementType, changeType, "Deprecation",
                    OldHeader?.Deprecated.ToString(), NewHeader?.Deprecated.ToString()));

            if (Style)
                returnList.Add(new ChangedInfoBO(elementType, changeType, "Style", OldHeader?.Style.ToString(),
                    NewHeader?.Style.ToString()));

            if (Explode)
                returnList.Add(new ChangedInfoBO(elementType, changeType, "Explode", OldHeader?.Explode.ToString(),
                    NewHeader?.Explode.ToString()));
