...
namespace LimeFlight.OpenAPI.Diff.Output
{
    public class ConsoleRender : IConsoleRender
    {
        ...
        private static string UlParam(ChangedParametersBO changedParameters)
        {
            ...
            var sb = new StringBuilder();
            foreach (var param in addParameters) sb.Append(ItemParam("Add ", param));
            foreach (var param in changed) sb.Append(LiChangedParam(param));
            foreach (var param in delParameters) sb.Append(ItemParam("Delete ", param));
            return sb.ToString();
        }
        ...
        private static string UlResponse(ChangedAPIResponseBO changedApiResponse)
        {
            ...
            var sb = new StringBuilder();
            foreach (var propName in addResponses.Keys) sb.Append(ItemResponse("Add ", propName));
            foreach (var propName in delResponses.Keys) sb.Append(ItemResponse("Deleted ", propName));
        ...
        {
            var sb = new StringBuilder();
            if (changedContent == null) return sb.ToString();
            foreach (var propName in changedContent.Increased.Keys) sb.Append(ItemContent("Added ", propName));
            foreach (var propName in changedContent.Missing.Keys) sb.Append(ItemContent("Deleted ", propName));
            ...
            bool isRequest)
        {
            ...
            sb.Append(ItemContent(title, contentType))
                ...
                .Append(Environment.NewLine);
            if (!changedMediaType.IsCompatible()) sb.Append(Incompatibilities(changedMediaType.Schema));
            return sb.ToString();
        }
        ...
        {
            var sb = new StringBuilder();
            if (schema.Items != null) sb.Append(Items(propName, schema.Items));
            ...
            foreach (var (name, value) in schema.ChangedProperties) sb.Append(Incompatibilities(prefix + name, value));
            return sb.ToString();
        }
        ...
        private static string Type(OpenApiSchema schema)
        {
            ...
            if (schema.GetSchemaType() == SchemaTypeEnum.ArraySchema)
                result = "array";
            else if (schema.Type != null) result = schema.Type;
            return result;
        }
    }
}