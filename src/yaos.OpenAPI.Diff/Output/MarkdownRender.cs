...
namespace yaos.OpenAPI.Diff.Output
{
    public class MarkdownRender : IMarkdownRender
    {
            ...
            changedOperations.ForEach(operation =>
                {
                    ...
                    if (ChangedBO.Result(operation.Parameters).IsDifferent())
                    {
                        details
                            ...
                            .Append(Parameters(operation.Parameters));
                    }
                    if (operation.ResultRequestBody().IsDifferent())
                    {
                        details
                            .Append(TitleH5("Request:"))
                            .Append(Metadata("Description", operation.RequestBody.Description))
        ...
        protected string Responses(ChangedAPIResponseBO changedApiResponse)
        {
            var sb = new StringBuilder("\n");
            sb.Append(ListResponse("New response", changedApiResponse.Increased));
            sb.Append(ListResponse("Deleted response", changedApiResponse.Missing));

            var all = changedApiResponse.Changed.Select(e => ItemResponse(e.Key, e.Value)).ToList();
        ...
        protected string ListResponse(string title, Dictionary<string, OpenApiResponse> responses)
        {
            var sb = new StringBuilder();

            var all = responses.Select(e => ItemResponse(title,e.Key, e.Value)).ToList();
            foreach (var s in all)
                sb.Append(s);
        ...
        protected string ItemResponse(string title, string code, OpenApiResponse response)
        {
            return ItemResponse(title, code, response.Description);
        }

        protected string ItemResponse(string code, ChangedResponseBO response)
        {
            var sb = new StringBuilder();
            sb.Append(ItemResponse(
                    "Changed response",
                    code,
                    null == response.NewApiResponse
                        ? ""
                        : response.NewApiResponse.Description));
            sb.Append(Headers(response.Headers));
            if (response.Content != null)
            {
                sb.Append(BodyContent(Li, response.Content));
            }
            return sb.ToString();
        }
        ...
        protected string ItemResponse(string title, string code, string description)
        {
            var sb = new StringBuilder();
            var status = "";
            if (!code.Equals("default") && int.TryParse(code, out var statusCode))
            {
                status =  ((HttpStatusCode)statusCode).ToString();
            }
            sb.Append($"{title} : **{code} {status}**\n");
            sb.Append(Metadata(description));
            return sb.ToString();
        }

        protected string Headers(ChangedHeadersBO headers)
        {
            var sb = new StringBuilder();
            if (headers != null)
            {
                sb.Append(ListHeader("New header", headers.Increased))
                    .Append(ListHeader("Deleted header", headers.Missing));

                var all = headers.Changed.Select(e => ItemHeader(e.Key, e.Value)).ToList();
                foreach (var s in all)
                    sb.Append(s);
            }
            return sb.ToString();
        ...
        protected string ListHeader(string title, Dictionary<string, OpenApiHeader> headers)
        {
            var sb = new StringBuilder();
            var all = headers.Select(e => ItemHeader(title, e.Key, e.Value)).ToList();
            foreach (var s in all)
                sb.Append(s);
        ...
        protected string ItemHeader(string title, string name, OpenApiHeader header)
        {
            return this.ItemHeader(title, name, header.Description);
        }

        protected string ItemHeader(string code, ChangedHeaderBO header)
        {
            return ItemHeader(
                "Changed header",
                code,
                null == header.NewHeader ? "" : header.NewHeader.Description);
        }
